...
begin {Setup}
  ...
  if Ctype = Combinations then
  begin
    NumberofSubsets := trunc(numberofSubsets / factorial(r))
  end;
  Count := 0;
end;
*)
...
begin
  ...
  begin
    ...
    begin
      j := n + 1
    end;    {drop out after max number created}
    if (j <= n) then
    begin
      repeat
        ...
        begin
          ...
          begin
            j := 1
          end; {Set initial trial number}
        end
        else
        begin
          Inc(j)
        end;                   {Otherwise try next number}
      until (i > r) or (j > n)
    end;            {loop 'til all have been tried}
    ...
    begin
      Done   := True;
      Result := True;
    end; {otherwise set for exit}
    if i = 0 then
    begin
      Done := True;
    end; {If we get to 0, all done}
  end;
end; {Getpermute}
*)
...
var
  i, j, k, UnusedCount, CurrentNum: integer;
  TempLoops: Bytearray;
  imult:     int64;
begin
  if not IsValidRSequence then
...
{******************** UnRank Functions ********************}
function TComboSet.UnRankR(const RPick, Number: integer;
  const NewCtype: TComboType; const Rank: int64):boolean;
begin
  case NewCtype of
    Combinations,
    CombinationsDown:
      Result := UnRankLexRCombo(RPick, Number, Rank);
    Permutations,
    PermutationsDown:
      Result := UnRankLexRPermute(RPick, Number, Rank);
    CombinationsCoLex,
    CombinationsCoLexDown:
      Result := UnRankCoLexRCombo(RPick, Number, Rank);
    PermutationsRepeat, PermutationsWithRep,
    PermutationsRepeatDown:
      Result := UnRankLexRepRPermute(RPick, Number, Rank);
    CombinationsRepeat, CombinationsWithrep,
    CombinationsRepeatDown:
      Result := UnRankLexRepRCombo(RPick, Number, Rank);
  else
    begin
      Result := False;
    end;
...
function TComboSet.UnRankCoLexRCombo(const RPick, Number: integer; const Rank: int64):boolean;
{
...
var
  i, x: integer;
  k, j, RankZeroBased: int64;
begin
  x := n;
  RankZeroBased := Rank - 1;
  if not (Ctype in [CombinationsCoLex, CombinationsCoLexDown]) then
    Ctype := CombinationsCoLex;
  Result := IsValidRNRank(RPick,Number,Rank,Ctype);
...
function TComboSet.UnRankLexRCombo(const RPick, Number: integer; const Rank: int64):boolean;
var
  i:    integer;
  k, j: int64;
begin
  k := 0;
  if not (Ctype in [Combinations, CombinationsDown]) then
  begin
    Ctype := Combinations;
  end;
  Result := IsValidRNRank(RPick,Number,Rank,Ctype);
  ...
  begin
    ...
    repeat
      ...
      j := Binomial(RPick - i, Number - Selected[i]);
    until (Rank <= (k + j));             {check rank with new binomial value}
  end;
  Selected[RPick] := Selected[RPick - 1] + rank - k;   {set remaining value to diff}
end;
...
begin
  if not (Ctype in [CombinationsRepeat,CombinationsWithrep, CombinationsRepeatDown]) then
  ...
  begin
    Selected[i] := Selected[i - 1];    {set to previous }
    While GetRepRCombo(RPick-i,DistinctNo) < RankRemaining do
    begin
      RankRemaining := RankRemaining - GetRepRCombo(RPick-i,DistinctNo);
      inc(Selected[i]);
      dec(DistinctNo);
    end;
  end;
  Selected[RPick] := Selected[RPick - 1] + RankRemaining - 1;   {set remaining value to diff}
end;

function TComboSet.UnRankLexRPermute(const RPick, Number: integer; const Rank: int64):boolean;
var
  i, jj, UnusedCount, ZeroBasedNum: integer;
  k, RankZeroBased: int64;
begin
  if not (Ctype in [Permutations, PermutationsDown]) then
  begin
    Ctype := Permutations
  end;
  Result := IsValidRNRank(RPick,Number,Rank,Ctype);
  ...
  r := RPick;         {set pick}
  ClearArrays;
  k := GetRPermute(RPick, Number) div Number;
  for i := 1 to RPick - 1 do
  begin
    ...
    k := k div (Number - i); {calculate new divisor}
  end; {for loop}
  ...
  Loops[jj] := 1;        {mark as used}
  Selected[RPick] := jj;       {update value}
end;

function TComboSet.UnRankLexRepRPermute(const RPick, Number: integer;
  const Rank: int64):boolean;
var
  i: integer;
  k, RankZeroBased: int64;
...
begin
  if not (Ctype in [PermutationsRepeat,PermutationsWithrep, PermutationsRepeatDown]) then
  ...
  r := RPick;
  ClearArrays;
  k := GetRepRPermute(RPick - 1, Number);
  for i := 1 to RPick - 1 do
  begin
    ...
    k := k div (Number);                                    {calculate new divisor}
  end;
  Selected[RPick] := RankZeroBased + 1;
end;
...
begin
  RandomRank := random(GetRCombo(RPick,Number))+1; {Math.RandomRange(1,GetRCombo(RPick,Number));}
  Result := UnRankLexRCombo(RPick,Number,RandomRank);
end;
...
RandomRank : int64;
begin
  RandomRank :=random(GetRCombo(RPick,Number))+1;  {Math.RandomRange(1,GetRepRCombo(RPick,Number));}
  Result := UnRankLexRepRCombo(RPick,Number,RandomRank);
end;
...
RandomRank : int64;
begin
  RandomRank := random(GetRCombo(RPick,Number))+1; {Math.RandomRange(1,GetRepRPermute(RPick,Number));}
  Result := UnRankLexRepRPermute(RPick,Number,RandomRank);
end;
...
RandomRank : int64;
begin
  RandomRank := random(GetRCombo(RPick,Number))+1; {Math.RandomRange(1,GetRPermute(RPick,Number));}
  Result := UnRankLexRPermute(RPick,Number,RandomRank);
end;
