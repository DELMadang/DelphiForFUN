...
uses
  ...
  MPlayer, StdCtrls, ComCtrls, Menus, CheckLst, ExtCtrls, shellAPI, Spin,
  Grids;
...
type
  ...
  TForm1 = class(TForm)
    ...
    DTMFSheet: TTabSheet;
    DTMFGrid: TStringGrid;
    ToneLenEdt: TSpinEdit;
    Label9: TLabel;
    Label10: TLabel;
    GapLenEdt: TSpinEdit;
    InputEdt: TEdit;
    Label11: TLabel;
    GenerateDTMFBtn: TButton;
    SaveDTMFBtn: TButton;
    Memo2: TMemo;
    ClearBtn: TButton;
    Label12: TLabel;
    ...
    procedure MetroBtnClick(Sender: TObject);
    procedure Edit2Change(Sender: TObject);
    procedure GenerateDTMFBtnClick(Sender: TObject);
    procedure SaveDTMFBtnClick(Sender: TObject);
    procedure DTMFGridClick(Sender: TObject);
  ...
  public
    ...
    datacount,TotDataCount,maxval:integer;
    ...
    metrodelay:integer;
    DTMFList:TStringList;

    {Calls AddComplexSound to make new memory stream in new (for compatibility with older code)}
    procedure MakeComplexSound(n:integer; freqlist:TStrings;
                           Duration{mSec}: Integer; Volume: TVolumeLevel);

    {Create or add to specified playable memory stream }
    procedure AddComplexSound(Newstream:boolean; N:integer {stream # to use};
              freqlist:TStrings; Duration{mSec}: Integer; Volume: TVolumeLevel);
...
var
  {column by column}
  DTMFdata:array [0..24] of string = (' ','697','770','862','941',
                                  '1209',' 1',' 4',' 7',' *',
                                  '1336',' 2',' 5',' 8',' 0',
                                  '1477',' 3',' 6',' 9',' #',
                                  '1633',' A',' B',' C',' D');
...
begin
  ...
  newa:=500;
  news:=0;
  DTMFList:=TStringlist.create;
  with DTMFGrid, DTMFList do
  for c:=0 to 4 do
  for r:=0 to 4 do
  begin
    cells[c,r]:=DTMFdata[5*c+r];
    if (c>0) and (r>0) then
    begin
      Id:=trim(cells[c,r])[1];
      newf:= StrtoInt(cells[0,r]);
      ff:=TFreqObj.create(newf,newp,newa,news);
      AddObject(Id+'1',ff);
      newf:= StrtoInt(cells[c,0]);
      ff:=TFreqObj.create(newf,newp,newa,news);
      AddObject(Id+'2',ff);
      DTMFGrid.objects[c,r]:=DTMFList;
    end
    else if (c=0) and (r=0) then
    begin {generate silence tone}
      Id:='  ';
      newf:= 0;
      ff:=TFreqObj.create(1,0,1,0);
      addobject(Id,ff);
      DTMFGrid.objects[c,r]:=DTMFList;
    end
    else DTMFGrid.objects[c,r]:=nil;{no object for the first row and column of grid}
  end;
end;
...
begin
  AddComplexSound(True, N,{stream # to use}
                        freqlist, Duration{mSec}, Volume);
end;
...
{********************* AddComplexSound **************}
procedure TForm1.AddComplexSound(Newstream:boolean; N:integer {stream # to use};
                           freqlist:TStrings;
                           Duration{mSec}: Integer;
                           Volume: TVolumeLevel);
  {writes complex tone defined by a list to memory stream N }
...
begin
  ...
  begin
    ...
    begin
      ...
      // inc(DataCount,(Duration * SampleRate) div 1000); {record "duration" ms at "samplrate" samps/sec}
  ...
  begin
    ...
    savepos:=position;
    position:=4;
    Riffcount:=38+Totdatacount;
    write(Riffcount, sizeof(riffcount));
    position:=42;
    Write(TotDatacount, sizeof(Totdatacount));
    position:=savepos;
  end;
...
begin
  ...
  begin
    n:=itemindex;
    itemindex:=listbox2.ItemAtPos(mousepos,true); //mousepos.y div  itemheight;
...
var
  bpm, errcode :integer;
begin
  val(edit2.text,bpm, errcode);
  if errcode=0 then  metrodelay:=60000 div bpm  {ms delay between clicks}
  else metrodelay:=0;
end;


procedure TForm1.MetroBtnClick(Sender: TObject);
var
  freqlist:TStrings;
  dur:integer;
  nextstream:integer;
...
begin
  ...
  begin
    ...
    begin
      caption:='Stop';
      Tag:=0;
      edit2change(sender);  {make sure repeat rate is set}
      loadsound('Click1.snd');
      checkplaying;
      (* {Alternative to Click1.snd}
      freqlist:=TStringlist.create;
      ff:=TFreqObj.create(250,0,500, 0);
      freqlist.addobject('250',ff);
      nextstream:=(streaminuse+1) mod 2;
      MakeComplexSound(nextstream, freqlist, 5, volbar.position);
      streaminuse:=nextstream;
      playing:=true;
      freqlist.free;
      *)
      repeat
        {MsPlaySound;}  playbtnclick(sender);
        sleep(metrodelay);
...
(*   {Version to play each digit as it is generated}
procedure TForm1.GenerateDTMFBtnClick(Sender: TObject);
var
  nextstream:integer;
  freqlist:TStringlist;
  i,j,index:integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if index >=0  then
        begin
          freqlist.addobject(id, objects[index]);
          nextstream:=(streaminuse+1) mod 2;
          with TFreqObj(objects[index]), memo2 do
          begin
            lines.add(format('%s: freq = %d, amp= %d',
            ['"'+id[1]+'" '+copy(id,2,length(id)),f,a]));
          end;
        end;
      end;
      memo2.lines.Add(' ');
...
var
  ...
  Byteval:byte;
begin
  SaveDTMFBtn.Enabled:=false;
  freqlist:=TStringlist.create;
  Byteval:=0;
  s:=uppercase(InputEdt.text);
  with DTMFList do
  begin
    totsecs:= length(s)*(tonelenedt.value+gapLenEdt.value)/1000;
    nextstream:=(streaminuse+1) mod 2;
    if not assigned(ms[nextstream]) then ms[nextstream]:=TMemorystream.create;
    ms[nextstream].SetSize(trunc(totsecs*sampleRate)+100);
    ms[nextstream].Clear;
    for i:=1 to length(s) do
    begin
      freqlist.clear;
      ...
      if freqlist.count>0 then
      begin
        dur:=ToneLenEdt.value;
        AddComplexSound(i=1{start new sound stream}, nextstream, freqlist, dur, Volbar.position);
        freqlist.clear;

        {Now add a "between tones" gap}
        id:='  ';
        index:=indexof(id);
        if index>=0 then freqlist.addobject(id, objects[index]);
        dur:=GapLenEdt.value;
        AddComplexSound(False{start new sound stream}, nextstream, freqlist, dur, Volbar.position);
        memo2.lines.add('Gap added');
        memo2.lines.Add(' ');
      end;
    end; {end "i" loop over input characters}

    stopbtnclick(sender);
    playing:=true;
    streaminuse:=nextstream;
    MsPlaySound;
    SaveDTMFBtn.Enabled:=true;
  end;
  freqlist.free;
end;

{************ SaveDMFBtnClick ************}
procedure TForm1.SaveDTMFBtnClick(Sender: TObject);
begin
  if savedialog1.execute then
  begin
    ms[streaminuse].savetofile(savedialog1.filename);
...
{************** DTMFGridClick **********}
procedure TForm1.DTMFGridClick(Sender: TObject);
begin
 with DTMFGrid do
 begin
   if (col>0) and (row>0) then
   begin
     inputedt.text:=inputedt.Text+trim(cells[col,row]);
   end;
 end;
end;
