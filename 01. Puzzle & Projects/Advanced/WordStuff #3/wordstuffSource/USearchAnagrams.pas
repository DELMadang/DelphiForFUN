...
type
  ...
  var
    SearchAnagrams:TSearchAnagrams;
...
var  possibles:TTestwords;
    ...
    var
      w:string;
    begin
      SearchAnagrams.init(ss,length(ss), length(ss), false,false,false);
      possibles[newletter]:='';
      begin
        if pubdic.dicloaded then
        If (Getnext(pubdic,w))  and (SearchAnagrams.tag=0)
...
var  possibles:TTestwords;
    ...
    begin
      ...
      begin {Called from Wordstuff}
        if pubdic.dicloaded then
        while (Getnext(pubdic,w))  and (SearchAnagrams.tag=0)
        do list.add(w)
        else if privdic.dicloaded then
        while Getnext(privdic,w)  and (SearchAnagrams.tag=0)
        do list.add(w);
      end;
    end;
