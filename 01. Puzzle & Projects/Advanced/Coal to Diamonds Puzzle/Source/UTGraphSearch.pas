...
interface
 ...
 type
  ...
  public
    ...
    directed two way (for weighted non-directed use same weights both directions)}
...
begin
  ...
  begin
    ...
    {solution found}
    begin
      If @verbosecall <> nil then verbosecall('5.  Destination node '+ strings[v]+' found!');
    ...
    begin
      ...
      If @verbosecall <>nil then verbosecall('2. Node '+strings[u]+' through node '
      ...
      begin  {so...}
       ...
       If @verbosecall<>nil then verbosecall('3. *** It is the smallest path so far from start to '+strings[u]);
      end;
    end;
    If @verbosecall<>nil then verbosecall('4. All paths from '+ strings[v]+' checked');
  end;
