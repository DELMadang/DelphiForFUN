unit U_Catapult4;
 {Copyright  © 2005, 2017 Gary Darby,  www.DelphiForFun.org
...
type
  ...
  public
    {common values}
     ...
     dir:string;  {program directory name}
     ...
     procedure getspringValues(angle:float; var xspring2,yspring2,springlength,sangle, fangle:float);
  end;
...
begin
  ...
  with positions[i] do
  begin
    memo1.lines.add(format('%5.3f           %6.3f         %6.3f         %6.3f     '
                     +'        (%6.3f,%6.3f)   (%6.3f,%6.3f) ',
        [time,theta/Pi*180,omega/Pi*180, alpha/Pi*180, xpos, ypos, xvel, yvel]));
    if springrbtn.checked then
    memo1.lines.add(format('   Spring -  (X,Y) (%6.2f,%6.2f), Length %6.2f,  Angle %6.2f, Force-Angle %6.2f',
            [xspring2, yspring2, springlength, springangle/Pi*180, forceangle/pi*180]));
  ...
  memo1.lines.add(format( 'Projectile fired at %4.1f degrees with velocity of %6.1f %s/sec',
                [90-positions[valcount].theta/pi*180,
                 positions[valcount].omega*dm,LName]));
  ...
  begin
    ...
    //offsety:=(catrect.bottom-catrect.top) div 3;
...
procedure TForm1.getspringValues(angle:float; var xspring2,yspring2,springlength,sangle, fangle:float);
  begin
...
{Next alpha , angular acceleration, cleed for eacg calulation point}
...
begin
  ...
  DF:=DFEdt.value;
  drag:=dragEdt.value;
  Pivotheight:=PivotHeightEdt.value;
  XSpring1:=XSpring1Edt.value;
  YSpring1:=YSpring1Edt.value;
  if   springRbtn.checked then
  begin
   {get coordinates of attach point}
    xspring2:=Df*cos(theta0);
    yspring2:=-DF*sin(theta0);
    {calc length of spring}
  ...
  begin
    Fangle:=Pi/2;
    if df>0 then Fangle:=-FAngle;
  end;

  K:=KEdt.value;
  Inertia:= Mass*DM*DM/3+ MB*L/3*(Intpower(DM/L,3)+Intpower((L-DM)/L,3));
...
begin
  ...
  ininame:=dir +'Catapult.ini';
  ini:=TInifile.create(ininame);
  catname:=dir+ini.readstring('General','Cat Filename','Sample.cat');
  ini.free;
