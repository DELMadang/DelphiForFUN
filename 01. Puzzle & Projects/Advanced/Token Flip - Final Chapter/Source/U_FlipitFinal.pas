...
{
Objective is to turn all tokens so that the white side
is up.  Each click will turn over the clicked token
...
var boardsize:integer=6;
type
  Flagrec= array[0..10,0..10] of boolean;
  Tmode=(play,build);
  TBoard=class(TObject)
     constructor create(newsize:integer);
     destructor destroy;  override;
   public
     b:Flagrec; {NxN array of boolean flags representing board positions, true = white side}
...
begin
  ...
  begin
    colcount:=boardsize;
    rowcount:=boardsize;
    defaultcolwidth :=boardwidth div  colcount -2;
    defaultrowheight:=boardheight div rowcount -2;
  end;
  adjustgridsize(drawgrid1);
  board:=TBoard.create(boardsize);
  saveboard:=TBoard.create(boardsize);
  board.init;
  newboardbtnclick(self);
...
begin
  randomize;
  boardwidth:=drawgrid1.width;
  boardheight:=drawgrid1.height;
  makenewboard;
end;
...
begin
  ...
  if getsolution then solution:=board else solution:=nil;{linear algebra solution}
  {use  following stmt instead for debugging}
  {if solveform.showmodal=mrOK then solution:=board else solution:=nil;}
  ...
  Begin
    ...
    with listbox1 do
    begin
      items.add('Move '+inttostr(i+1)+': (Col:'+inttostr(path[i].x+1)
...
{Make a new random board that can be solved in the specified number of moves
 (or less)}
var
  i, nbrmoves :integer;
