...
{add variable and value or update value only}
var
  s:string;
  index:integer;
  mr:integer;
begin
  ...
  begin
    ...
    begin
      if not silent then
      begin
        showerror('Variable addition for '+newvarname+' failed.');
      end;
      result:=false;
    end;
  end
  else varlist.objects[index]:=TObject(newvarval);
end;
...
{also builds the postfix list in the process}
         ...
         Function makeOpRec(const t:string):Topobj;
           ...
           begin
             ...
             begin
               ...
               if index<0 then
               begin
                 showerror('Variable '+t+' not defined');
...
{Evaluate a single expression in postfix form, returns true if satisfied}
  ...
  Begin
    ...
    begin
      showerror('System error, too many operands');
    end;
