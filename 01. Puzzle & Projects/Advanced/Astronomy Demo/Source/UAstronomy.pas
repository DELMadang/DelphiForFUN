...
public
  ...
  function AngleToStr(angle:extended; decimal:boolean):string;   overload;
  ...
  {Call conversion types for GenCom}
   ...
   longtimenames:array[ttlocal..ttLST] of string=
       ...
       begin
         ...
         result:=format('-%3d° %2d´ %5f´´',[d,trunc(m),s])
         else result:=format('%3d° %2d´ %5.2f´´',[d,trunc(m),s]);
       end;

       {*** AngletoStr (with decimal format test) ***}
       function AngleToStr(angle:extended; decimal:boolean):string;
       var
         D:integer;
         M,S:extended;
       begin
       ...
       begin
         n:=pos(' ',s);
         if n=0 then n:=pos(',',s);
         if n=0 then n:=pos(deg,s);
         if (n=0) or (s[length(s)]=deg) then
         begin
           angle:=strtofloatdef(s,1000);
           result:=angle<>1000;
         end
         else
         begin
...
var
  ...
  SR:extended;
begin
  T:=(FJDate+int(funiversaltime))/36525.0+(Frac(FUniversalTime)*24/8.766e5);
  T2:=T*T;
  A:=1.000021359E2*T;
  B:=360*(FRAC(A));
