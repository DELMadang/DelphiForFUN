...
type
  ...
  TTrackPoint=record  {record used in Rpoints array to describe track}
    x,y:float;
    angleright, sinangleright,cosangleright:float;
  ...
  TCircle=class(TObject)   {circle template class}
    Center:TPoint;
    Radius:integer;
    NewRadius:integer;
    Newcenter:TPoint;
  ...
  TCoaster=class(TPaintbox)
    ...
    public
      ...
      g:float; {G force}
      fward:boolean;
      ouchresources: array [0..5] of string; {crash sounds}
      ...
      function PixelToVirtual(x,y:float):TRealpoint;
      function VirtualToPixel(x,y:float):TRealpoint;
...
constructor TLine.create(newPBox:TPaintBox);
begin
  inherited create;
  PBox:=newPBox;
  l1:=point(0,0);
...
procedure TLine.savetostream(st:Tstream);
begin
  with st do
  begin
...
procedure TLine.loadfromstream(st:Tstream);
begin
  with st do
  begin
...
constructor TCircle.create(newPBox:TPaintBox);
begin
  inherited create;
  PBox:=newPBox;
...
procedure TCircle.savetostream(st:Tstream);
begin
  with st do
  begin
...
procedure TCircle.loadfromstream(st:Tstream);
begin
  with st do
  begin
...
begin
  ...
  begin
    ...
    else if c is TCircle then
    with c as TCircle do
    begin
...
begin
  templatelist.add(Tcircle.create(self));
...
begin
  templatelist.add(TLine.create(self));
  curobj:=templatelist.items[templatelist.count-1];
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if tobject(list[j]) is tline then TLine(list[j]).draw
            else if tobject(list[j]) is tcircle then TCircle(list[j]).draw;
          end;
   ...
   normal:
     begin
       ...
       begin
         ...
         begin
           if Tobject(templatelist[i]) is tline then
           with TLine(templatelist[i]) do
           begin
             d1:=dist(l1,p);
             d2:=dist(l2,p);
           ...
           if Tobject(templatelist[i]) is tcircle then
           with TCircle(templatelist[i]) do
           begin
             d1:=dist(center,p);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            TLine(c).savetostream(st);
          end
          else if c is TCircle then
          begin
            ch:='C';
            writebuffer(ch,sizeof(ch));
            TCircle(c).savetostream(st);
          end;
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          c:=tLine.create(self);
          Tline(c).loadfromstream(st);
          templatelist.add(c);
        end
        else if ch='C' then
        begin
          c:=tCircle.create(self);
          TCircle(c).loadfromstream(st);
          templatelist.add(c);
        end;
      end;
    end;
...
{******************** PixelToVirtual ****************}
function TCoaster.PixelToVirtual(x,y:float):TRealPoint;
begin
...
{******************** VirtualToPixel *****************}
function TCoaster.VirtualToPixel(x,y:float):TRealPoint;
begin
