...
type
 ...
 TDic=class
    ...
    Letterindex:array ['a'..succ('z')] of integer;
    ...
    constructor Create(Precheckedflag:boolean);
    ...
    function Lookup(const s:string; var abbrev,foreign,caps:boolean):boolean; overload;
    function Lookup(const s:string):boolean; overload;
    ...
    function GetnextWord(var word:string):boolean; overload;
...
Uses Inifiles;
   ...
   {replace initial letters of word that match prev word with a count
    field}
       ...
       Begin
         ...
         prevlen:=length(prev);
         i:=1;
         while (i<=min(len,prevlen)) and (word[i]=prev[i]) do inc(i);
         letterstocopy:=i-1;
 ...
 Begin
  ...
  For i:='a' to 'z' do letterindex[i]:=0;
  CurrentWordIndex:=-1;
  IndexDirty:=true;
  DicDirty:=false;
  MaxWordLength:=0;
 ...
 function getword(var w:string):string;
    ...
    Begin
      ...
      {get to first letter}
      while (i<=length(w)) and  (not (w[i] in ['a'..'z'])) do inc(i);
      if w[i] in ['a'..'z'] then
      begin
        If i>1 then w:=copy(w,i,length(w)-i+1);
        i:=1;
        while (i<=length(w)) and  ((w[i] in ['a'..'z'])) do inc(i);
        result:=copy(w,1,i-1);
        system.delete(w,1,i);
      End
      else w:='';
 ...
 Begin
   ...
   ini:=TIniFile.create(path+'Dic.ini');
   DefaultDic:=ini.ReadString('Files','Default Dictionary',path + 'Full.Dic');
 ...
 Begin
   path:=extractfilepath(application.exename);
   ini:=TIniFile.create(path+'Dic.ini');
   SmallDic:=ini.ReadString('Files','Small Dictionary',path + 'Small.Dic');
 ...
 Begin
   path:=extractfilepath(application.exename);
   ini:=TIniFile.create(path+'Dic.ini');
   MediumDic:=ini.ReadString('Files','Medium Dictionary',path + 'General.Dic');
 ...
 Begin
   path:=extractfilepath(application.exename);
   ini:=TIniFile.create(path+'Dic.ini');
   LargeDic:=ini.ReadString('Files','Large Dictionary',path + 'Full.Dic');
...
Begin
  for a:='a' to succ('z') do Letterindex[a]:=-1;
  prevletter:=pred('a');
  letter:='a';
  maxwordlength:=0;
  abbrevcount:=0;
  ...
  Begin
    ...
    if letterstocopy=0 then letter:=words[i][2];
  ...
  End;
  if letter<>'z'
  then for a:=letter to 'z' do letterindex[a]:=words.count-1
  else letterindex[succ('z')]:=words.count;
  indexdirty:=false;
End;
...
Begin
  ...
  repeat
    ...
    if (currentwordindex>=Letterindex[succ(endletter)]) then exit;
...
Function TDic.GetnextWord(var word:string):boolean;
...
Begin
  ...
  repeat
    ...
    if currentwordindex>words.count-1 then exit;
    currword:=words[CurrentWordIndex];
    currword:=expandword(prevword,currword,a,f,c);
    If (currword[1]>endletter) then exit;
    If (startlength<=length(currword)) and (length(currword)<=endlength)
    and (not (a or f or c))
    then
...
{***************** LookUp *****************}
Function TDic.Lookup(const s:string; var abbrev,foreign,caps:boolean):boolean;
...
var
  ...
  if prechecked or IsValidword(ss) then
  Begin
    start:=ss[1];
    len:=length(ss);
    saverange;
    ...
    Begin
      ...
      if (testword>=ss) then belowit:=false;
    End;
    if (testword=ss) and (origlen=len) then result:=true;
    restorerange;
  End;
End;

function TDic.lookup(const s:string):boolean; 
{overloaded lookup to ingore abbreviations and  foreign words}
var
  a,f,c:boolean;
begin  
  result :=lookup(s,a,f,c);
  if result then result:= (not a) and (not f);
end;
