...
type
  ...
  public
    ...
    function sunpos( longitude, latitude, hrtime:extended;
                 tz:extended; day:integer; var pos:T3DVector):boolean;
...
{****************************   SUNPOS *****************************}
function TForm1.sunpos( longitude, latitude, hrtime, tz:extended;
                 day:integer;
    ...
    begin
      ...
      add('Solar noon: ' + timetostr(midday/24)+', Azimuth: '
         ...
         add(format('Eq of time:  %4.2f minutes',[EqOfTimeMinutes])
                  +' This is the amount that sun position varies from average because '
  ...
  begin
    ...
    begin
      ...
      t:=int(sunrise+1);  {next hour after sunrise}
       while t<= sunset do
       begin
         {if tag>0 then} sunpos(long,lat,T,tzHours, Julianday(datepicker.date),pos);
         //else sunposOld(long,lat,T,tzHours, Julianday(datepicker.date),pos);
         ...
         //memo2.lines.add(format('T:%4.1f, Az:%5.1f, Alt:%5.1f,HA:%4.1f',[t,az,alt,hourangle]));
         t:=t+1;
       end;
       {restore real values}
       sunpos(long,lat,Timepicker.time*24,
    ...
    else if (drawmode=analemma) and (length(anpoints)>0) then
    begin
      brush.color:=clblue;
      pen.color:=clblack;
...
{*********** GetSystimeBtn *******}
procedure TForm1.GetSystimeBtnClick(Sender: TObject);
var
...
procedure TForm1.PlotLastClick(Sender: TObject);
begin
