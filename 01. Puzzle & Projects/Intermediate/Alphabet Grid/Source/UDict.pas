...
type
  ...
  MyString={ANSI}String;  {use default Unicode}
  MyChar={ANSI}Char;
 ...
 TDic=class(TObject)
    ...
    IniPathName:MyString;  {path to dic.inifile}
    Maxwordlength:integer;
    DicLoaded:boolean;
    Dicname:MyString;
    DefaultDic,SmallDic,MediumDic,LargeDic:MyString;

    constructor Create(Precheckedflag:boolean); overload;
    constructor Create(Newpath:MyString; Precheckedflag:boolean); overload;

    procedure LoadDicFromFile(filename:MyString);
    procedure LoadDefaultDic;
    procedure LoadSmallDic;
    procedure LoadMediumDic;
    procedure LoadLargeDic;

    procedure SaveDicToFile(filename:MyString);
    procedure SaveDicToTextFile(filename:MyString); {save uncompressed}

    procedure Setrange(const letter1:MyChar;length1:byte;const letter2:MyChar;length2:byte);
    procedure saverange;
    procedure restorerange;

    function Lookup(s:MyString; var abbrev,foreign,caps:boolean):boolean;  overload;
    Function Lookup(s:MyString ):boolean; overload;
    function GetnextWord(var word:MyString;var abbrev,foreign,caps:boolean):boolean; overload;
    Function GetnextWord(var word:MyString;var wordnbr:integer; var abbrev,foreign,caps:boolean):boolean; overload;
    function GetWordByNumber(n:integer; var word:MyString):boolean;
    function IsValidword(const ss:MyString):boolean;
    function AddWord(s:MyString;abbrev,foreign,caps:boolean):boolean;
    function RemoveWord(s:MyString):boolean;
    ...
    Function LookupPartial(s:string; shortest, longest:integer):boolean;
  End;
...
Uses Inifiles;
 ...
 Function min(a,b:integer):integer; Begin If a<b then result:=a else result:=b; End;
  ...
  Function CompressWord(prev,word:MyString; abbrev,foreign,caps:boolean):MyString;
   {replace initial letters of word that match prev word with a count
    field}
       ...
       Begin
         ...
         result[1]:=MyChar(letterstocopy or mask);
       End;
    ...
    Function ExpandWord(prev,Compressedword:MyString;Var abbrev,foreign,caps:boolean):MyString;
    {expand a compressed word  to full length}
    var
      n:integer;
    Begin
      n:=getasciicode(Compressedword[1]);

      result:=copy(prev,1,ord(n {ord(Compressedword[1]}) and $0F)
            + copy(compressedword,2,length(compressedword)-1);
      if (n {byte(compressedword[1])} and $40)>0 then abbrev:=true else abbrev:=false;
      if (n {byte(compressedword[1])} and $20)>0 then foreign:=true else foreign:=false;
      if (n {byte(compressedword[1])} and $10)>0 then caps:=true else caps:=false;
    End;
 ...
 {*************** TDic.Create ********************}
 Constructor TDic.Create(newpath:MyString; precheckedflag:boolean);
 {create dictionary object}
 var
   i:MyChar;
   ini:TInifile;
 ...
 Procedure TDic.LoadDicFromFile(filename:MyString);
 {load a dictionary}

 function getword(var w:MyString):MyString; overload;
    var
      i:integer;
    Begin
      i:=1;
 ...
 function getword(const ww:MyString; var startat:integer):MyString; overload;
    var
      i:integer;
      w:MyString;
    Begin
...
begin
 ...
 {******************** TDic.SaveDicToFile *******************}
 Procedure TDic.SaveDicToFile(filename:MyString);
 var
   f:text;
   w:MyString;
   a,fr,caps:boolean;
 ...
 Procedure TDic.SaveDicToTextFile(filename:MyString);
 {save dictionary}
 var
   textname:MyString;
   f:textfile;
...
Begin
  ...
  Begin
    {For XE: Get the true ASCII code (the number of characters to use from the previous
     word) which may had been converted to a 2-byte character by new string handling}

    //setlength(SStr,length(words[i]));
    //sstr:=shortstring(words[i]);
    //n:=ord(sstr[1]);
    n:=getAsciiCode(words[i]);
    ...
    wordlength:=letterstocopy+length(s {words.strings[i]})-1;
    if wordlength>maxwordlength then maxwordlength:=wordlength;

    if letterstocopy=0 then
    begin
      letter:=s[2];  {new letter}
    end;
...
function lowcase(c:MyChar):MyChar;
begin   result:=MyChar(ord(c) or $20); end;
...
Function TDic.GetnextWord(var word:MyString;var abbrev,foreign,caps:boolean):boolean;
...
Function TDic.GetnextWord(var word:MyString;var wordnbr:integer; var abbrev,foreign,caps:boolean):boolean;
...
{****************** TDic.IsValidword ********************}
Function TDic.Isvalidword(const ss:MyString):boolean;
...
Function TDic.GetWordByNumber(n:integer; var word:MyString):boolean;
...
{***************** LookUp *****************}
Function TDic.Lookup(s:MyString; var abbrev,foreign,caps:boolean):boolean;
...
Function TDic.Lookup(s:MyString):boolean;
{lookup word "s" in the current dictionary}
...
{****************** AddWord *****************}
Function TDic.AddWord(s:MyString; abbrev,foreign,caps:boolean):boolean;
{Add word s to dictionary}
var
  newword, nextword,w:MyString;
  a,f,c:boolean;
Begin
  s:=lowercase(s);
...
{****************** RemoveWord *****************}
Function TDic.RemoveWord(s:MyString):boolean;
...
{****************** SetRange ********************}
Procedure TDic.Setrange(const letter1:MyChar;length1:byte;
                        const letter2:MyChar;length2:byte);
...
Begin
  ...
  startletter:=lowcase(letter1);
  endletter:=lowcase(letter2);
  startlength:=length1;
  endlength:=length2;
...
(*
 Procedure TDic.Findwords(scrambled:MyString; NbrToFind:byte; List:Tstrings);
