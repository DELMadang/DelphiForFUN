...
{procedure Register;}
  TDeck = class(Tobject)
    DeckObj:array [0..51] of TCard; {TCard is graphics image component for card}
    NextCard:integer; {Card nbr of next card}
    DeckLoc:TPoint; {Top left corner of deck}
    constructor create(Aowner:TForm; newloc:TPoint);
    destructor destroy; override;
...
begin
  ...
  (*
    {commented by GDD 10/1/00 - not sure what they do - no visible change}
...
var
  i:integer;
Begin
  for i:=0 to 51 do deckobj[i].free;
  inherited;
end;
...
begin
  ...
  Nextcard:=51; {set initial card to top of deck}
...
begin
  ...
  Begin
    ...
    result:=true;
    dec(nextcard); {move the nxtcard pointer down}
    If nextcard>=0 then
