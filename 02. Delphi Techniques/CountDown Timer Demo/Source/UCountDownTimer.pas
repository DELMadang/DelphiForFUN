...
The fireds and methods in the "Published" and "Public" sections are available to
the programmer.
...
{Version 2 (June 2006) adds a feature to allow the timer to run up or down.  It
 also displays the time since the the timer was started rather than depending on
 1 second timer updates.  The previous version could "lose time" if the computer
 CPU was too busy to allow updates at one second intervals.

 Version 2.1 adds an "OnTimerPop" to allow calling programs access to timer
 fields, including current time value, once per second.
}
...
begin
  ...
  begin
    ...
    updatetimer;
    if Fcurrenttime <= 0.5/86400 then  {Within 1/2 second of zero}
    begin
